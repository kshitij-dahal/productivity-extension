(function() {
  ...
  document.addEventListener("DOMContentLoaded", function(event) {
    ...
    var total_remaining_time_element = document.querySelector(
      "#timer_container"
    );
    ...
    chrome.storage.onChanged.addListener(function(changes, namespace) {
      if (changes.hasOwnProperty("goal")) {
        timer_goal = changes["goal"].newValue;
      }
      console.log("CHANGED TIMER GOAL AT THIS MOMENT" + timer_goal);
    });
    ...
    document
      ...
      .addEventListener("click", function(event) {
        ...
        if (temp_pomodoro_option > 0) {
          ...
          pomodoro_on_or_off_element.appendChild(
    ...
    function establish_connection(port) {
      ...
      port.onMessage.addListener(function(msg) {
        if (msg.id == "newtab_opened" || msg.id == "goal_set") {
          var required_vals = ["goal", "goal_set_timer_values", "pomodoro"];
          chrome.storage.sync.get(required_vals, function(result) {
            if (msg.id == "goal_set") {
              local_goal_set_timer_values =
                ...
                msg.timer_min * 60 +
                msg.timer_sec;
              console.log(Math.round(timer_goal * 60 * 60));
              console.log(local_goal_set_timer_values + "oooOOOODAOSOGO");
    ...
    function timer_goal_accomplished() {
    ...
    function update_initial_values(msg, result) {
      ...
      var temp_rem_seconds =
        local_goal_set_timer_values -
        msg.timer_hr * 60 * 60 -
        msg.timer_min * 60 -
        msg.timer_sec;
      remaining_hr = parseInt(temp_rem_seconds / 3600);
      remaining_min = parseInt((temp_rem_seconds + 59) / 60) % 60;
